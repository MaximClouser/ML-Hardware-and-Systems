...
def linear_quantize(input, scale, zero_point):
    ...
    """
    # TODO: fill-in (start)
    output = ste_round.apply(input / scale) + zero_point
    # TODO: fill-in (end)
    return output
...
class SymmetricQuantFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, k, specified_scale=None, specified_zero_point=None):
        ...
        # TODO: fill-in (start)
        quantized = linear_quantize(x, scale, zero_point)
        output = torch.clamp(quantized, -2**(k-1), 2**(k-1)-1)
        # TODO: fill-in (end)
        return output
...
class AsymmetricQuantFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, k, specified_scale=None, specified_zero_point=None):
        ...
        # TODO: fill-in (start)
        quantized = linear_quantize(x, scale, zero_point)
        output = torch.clamp(quantized, 0, 2**k - 1)
        # TODO: fill-in (end)
        return output
    ...
    def get_quantization_params(self, saturation_min, saturation_max):
        ...
        with torch.no_grad():
            if self.is_symmetric:
                ...
                zero_point = ste_round.apply(-saturation_min / scale)
                # TODO: fill-in (end)
